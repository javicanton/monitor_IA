    ...
    def ensure_local_parquet(self, force: bool = False) -> str:
        with self.lock:
            ...
            if remote_meta is None:
                raise FileNotFoundError(f"No se encontró {self.parquet_key} en S3")
            meta = self._read_meta()
            local_meta = meta.get("messages_parquet", {})
            if force or not os.path.exists(self.messages_parquet_path) or local_meta != remote_meta:
                s3_client = self._get_s3_client()
                s3_client.download_file(self.parquet_key, self.messages_parquet_path)
                meta["messages_parquet"] = remote_meta
            self._sync_optional_file(Config.TOPICS_ASSIGNMENTS_KEY, self.assignments_csv_path, meta, "topics_assignments")
